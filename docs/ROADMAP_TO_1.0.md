...
- [x] 定义 `ExperimentSpec` 和可计算的设计要求。
- [x] 支持增益、带宽、截止频率、上升时间、过冲、纹波、功耗等自动测量。
- [x] 增加参数扫描、容差、温度和 Monte Carlo 分析。
- [x] 输出逐项 PASS/FAIL/未验证结果，不根据缺失数据猜测。
- [x] 支持理论值、仿真值与误差的结构化比较。

## 第四阶段：高价值元件、仪器与正式报告（已完成）

- [x] 增加变压器、电位器、继电器、晶振及常用功率半导体适配器。
- [x] 增加 D/T 触发器、计数器、移位寄存器和混合信号器件。
- [x] 增加万用表、Bode Plotter 和 Logic Analyzer。
- [x] 输出独立 HTML/PDF、中英双语报告和可复现 `manifest.json`。
- [x] 完成公开元件适配器接口、贡献模板和兼容性矩阵。
