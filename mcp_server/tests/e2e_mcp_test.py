...
async def main() -> None:
    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            sample = Path(configured_sample) if configured_sample else (
                RLC_SAMPLE if RLC_SAMPLE.exists() else LPF_SAMPLE
            )
            ...
            report["open_circuit"] = await call(session, "open_circuit", {"path": str(sample)})
            report["circuit_info"] = await call(session, "circuit_info")
            report["enum_components"] = await call(session, "enum_components", {"component_type": 0})
            ...
            outputs = outputs_result.get("outputs", []) if isinstance(outputs_result, dict) else []
            ...
            report["generate_report"] = await call(
                ...
                "generate_report",
                {
                    ...
                    "analyses": [report["run_dc"], report["run_ac"], report["run_transient"]],
            ...
            for key, tool, args in [
                ("netlist", "report_netlist", {"path": str(OUT / "e2e_circuit.netlist"), "fmt": 0}),
                ("bom", "report_bom", {"path": str(OUT / "e2e_circuit.bom"), "fmt": 0}),
                ("image", "get_circuit_image", {"path": str(OUT / "e2e_circuit.png"), "image_format": 2}),
            ]:
                try:
            ...
            try:
                report["save"] = await call(session, "save_circuit", {"path": str(OUT / "e2e_saved.ms14")})
            except RuntimeError as exc:
            ...
            for key in ("run_dc", "run_ac", "run_transient"):
                if not report[key].get("ready"):
                    raise AssertionError(f"{key} did not produce ready output: {report[key]}")
